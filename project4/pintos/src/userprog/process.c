...
#define frame_release(x) palloc_free_page(x)
#endif
...
static void file_close_all(void) {
...
static void release_children(void) {
    struct thread *cur = thread_current();
...
static void close_executing_file(void) {
    struct thread *cur = thread_current();
...
static void destroy_pagedir(void) {
    struct thread *cur = thread_current();
...
#ifdef VM
    supplemental_table_destroy(cur->supt);
    cur->supt = NULL;
#endif
...
#ifdef VM
  t->supt = supplemental_table_create ();
#endif
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (! supplemental_filesys_install(curr->supt, upage,
...
#ifdef VM
  success = success && supplemental_frame_install (t->supt, upage, kpage);
  if(success) vm_frame_eviction_select(kpage);
#endif
  return success;
}
